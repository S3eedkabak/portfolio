      ...
      <div className="game-panel">
        <div className="game-topbar">
          ...
          <div>
            ...
            <strong>
              {String(score).padStart(2, "0")} / {HITS_TO_WIN}
            </strong>
          </div>

          <button onClick={start}>
            {running ? t.game.restart : t.game.start}
          </button>
        </div>
        ...
        <div className="game-arena">
          ...
          {!running && !won && !lost && (
            <div className="game-state">
              <strong>{t.game.ready}</strong>
              <span>{t.game.readyText}</span>
              <button onClick={start}>{t.game.startRun}</button>
            </div>
          )}

          {lost && (
            <div className="game-state">
              <strong>{t.game.timeout}</strong>
              <span>
                {score} {t.game.secured}
              </span>
              <button onClick={start}>{t.game.runAgain}</button>
            </div>
          )}

          {won && (
            <div className="game-state is-success">
              <strong>{t.game.granted}</strong>
              <span>{t.game.ticketUnlocked}</span>
            </div>
          )}
        </div>
